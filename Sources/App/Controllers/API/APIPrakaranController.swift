...
struct APIPrakaranController: RouteCollection {
    ...
    func index(req: Request) async throws -> [Prakaran.Public] {
        ...
        let prakarans = try await query.all()
        return prakarans.map { Prakaran.Public(from: $0, chaupaiCount: $0.chaupais.count) }
    }
